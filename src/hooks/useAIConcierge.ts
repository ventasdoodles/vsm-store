...
import {
    useConciergeAudio,
    useConciergeSmokeAudit,
    buildNoWriteSmokeAuditSummary,
    extractNoWriteSmokeMetadata,
    NO_WRITE_SMOKE_AUDIT_FIELD,
    NO_WRITE_SMOKE_EDGE_METADATA_PRESENT_FIELD,
    NO_WRITE_SMOKE_REQUEST_CONTRACT_PRESENT_FIELD,
    NO_WRITE_SMOKE_REQUEST_FIELD,
...
export function useAIConcierge() {
    ...
    const { runNoWriteSmoke } = useConciergeSmokeAudit({
        user,
        authLoading,
        addMessage,
        setIsOpen,
    ...
    const { isListening, startRecording, stopRecording } = useConciergeAudio({
