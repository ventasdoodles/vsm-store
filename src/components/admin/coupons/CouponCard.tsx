...
export function CouponCard({ coupon, onEdit, onDelete, onDuplicate }: Props) {
    ...
    if (!coupon.is_active) status = { label: 'Inactivo', color: 'text-theme-secondary', bg: 'bg-gray-500/20', border: 'border-theme/30', icon: AlertCircle };
    ...
    else if (isScheduled) status = { label: 'Programado', color: 'text-blue-400', bg: 'bg-accent-primary/20', border: 'border-blue-500/30', icon: Clock };
    ...
    return (
        <div className={`rounded-2xl border ${status.border} bg-theme-primary/20 p-5 relative overflow-hidden group transition-all hover:border-theme`}>
            {/* Status Badge */}
            <div className={`absolute top-4 right-4 flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${status.bg} ${status.color}`}>
