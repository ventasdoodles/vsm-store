        ...
        return (
            <div className="rounded-[2rem] border border-white/5 bg-surface-base/50 p-6 flex items-center justify-center min-h-[120px] shadow-inner">
                 <div className="text-center">
                    ...
                    <p className="text-2xs text-theme-secondary">El motor analítico requiere más datos.</p>
                </div>
            </div>
        ); 
    }
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {data.topCategories.map((cat: { name: string; count: number }, i: number) => {
                                ...
                                return (
                                    <div key={i} className="group flex items-center gap-2 bg-surface-card/80 border border-white/5 px-3 py-1.5 rounded-lg hover:border-purple-500/30 transition-colors">
                                        ...
                                        <span className="text-2xs font-bold text-theme-secondary/50 bg-white/5 px-1.5 py-0.5 rounded-md">
                                            x{cat.count}
                                        </span>
                                    </div>
                                );
                        ...
                        <div className="space-y-2">
                            {data.topProducts.map((prod: { name: string; count: number }, i: number) => (
                                <div key={i} className="flex items-center justify-between bg-gradient-to-r hover:from-white/5 hover:to-transparent p-2.5 rounded-xl border border-transparent hover:border-white/5 transition-all group">
                                    ...
                                    <span className="text-2xs font-black text-fuchsia-400 bg-fuchsia-400/10 border border-fuchsia-400/20 px-2.5 py-1 rounded-md whitespace-nowrap flex items-center gap-1 shadow-sm">
