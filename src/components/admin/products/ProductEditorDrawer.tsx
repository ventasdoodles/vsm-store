...
import { useAdminProductDetail, type ProductFormData, uploadProductImage } from '@/hooks/admin/useAdminProducts';
import { enrichProduct, type EnrichmentPackage } from '@/services/admin';
...
export function ProductEditorDrawer({
    ...
    useEffect(() => {
        ...
        setApprovedFields(new Set(['short_description', 'description', 'ai_sales_note', 'specs', 'tags']));
    ...
    const handleAIGenerate = async () => {
        ...
        try {
            ...
            setApprovedFields(new Set(['short_description', 'description', 'ai_sales_note', 'specs', 'tags']));
            notify.success('Enriquecimiento listo', 'Revisa las sugerencias y aplica las que quieras.');
        } catch (_err) {
    ...
    const handleApplyEnrichment = () => {
        ...
        setFormData(prev => {
            ...
            if (approvedFields.has('short_description') && enrichmentResult.short_description) {
                updated.short_description = enrichmentResult.short_description;
            }
            if (approvedFields.has('description') && enrichmentResult.description) {
                updated.description = enrichmentResult.description;
            }
            if (approvedFields.has('ai_sales_note') && enrichmentResult.ai_sales_note) {
            ...
            if (approvedFields.has('tags') && enrichmentResult.tags?.length) {
                updated.tags = Array.from(new Set([...(prev.tags || []), ...enrichmentResult.tags]));
            }
        ...
        setEnrichmentResult(null);
        notify.success('Aplicado', 'Los campos aprobados fueron aplicados al formulario.');
    };
