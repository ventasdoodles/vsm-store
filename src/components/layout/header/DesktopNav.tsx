...
import { Flame, Leaf, Truck, Tag, TicketPercent, Sparkles, PackageCheck } from 'lucide-react';
...
export function DesktopNav({ compact = false }: DesktopNavProps) {
    ...
    return (
        <nav aria-label="Navegación principal" className="hidden lg:flex items-center justify-center gap-1.5 xl:gap-2 flex-shrink-0 p-1.5 rounded-full bg-[#1e2538]/60 backdrop-blur-md border border-white/10 shadow-inner shadow-black/20 transition-all hover:bg-[#1e2538]/80 hover:border-white/20">
            ...
            >
                <Sparkles className="h-4 w-4 relative z-10 group-hover:text-blue-400 transition-colors" />
                <span className="relative z-10 tracking-wide">Lo Nuevo</span>
            </Link> 
            ...
            {isAuthenticated && (
                <>
                    ...
                    >
                        <PackageCheck className="h-4 w-4 relative z-10 group-hover:text-blue-400 transition-colors" />
                        <span className="relative z-10 tracking-wide">Mis Compras</span>
                    </Link>
                </>
            )}
