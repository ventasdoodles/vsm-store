...
*Ultima actualizacion: 6 de mayo de 2026 (Tracking Readiness).*
  - **Verdict:** NOT_READY_TRACKING_SCHEMA_AMBIGUITY.
  - **Facts:** Admin tracking controls write `orders.tracking_notes` but not `orders.tracking_number`. `tracking_notes` is used as free text/link in UI, while `tracking_number` is the guide field for storefront/Césarín logic. No carrier field exists on orders; DHL uses separate Edge Function. No side effects (notifications, inventory, MP) found for tracking updates. VSM-0038/VSM-0039 untouched. Future smokes require deciding canonical field semantics.
  - **Explicit Non-claims:** Does NOT claim full tracking readiness. Does NOT claim `tracking_notes`/`tracking_number` sync. Does NOT claim Césarín uses `tracking_notes` as guide truth. Does NOT claim tracking UI mutation validated. Does NOT claim shipped/delivered readiness, notifications/emails, or DHL tracking readiness.
