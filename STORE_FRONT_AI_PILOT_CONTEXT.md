...
- Deprecated `public_web_search_legacy` / `public_url_context_legacy` remain compatibility-only rather than the active primary path.
...
- Non-claims remain explicit: this is Wave 6 Pass 1 plus Pass 2 only, not full web-intelligence completion, not a citation framework, not a planner/orchestrator redesign, not live/voice, and not admin / Cesarin OS drift.
...
- Deprecated legacy web helpers remain compatibility-only rather than the active primary path.
