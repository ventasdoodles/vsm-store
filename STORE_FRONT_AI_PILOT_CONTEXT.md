...
3. **Verify:** Check if the assistant follows the Sommelier persona rules, asks one commercially useful narrowing question when needed, only steers toward one option when the comparative support is real, keeps post-choice confidence language modest and supported, and matches handoff strength to real branch support. Weak fallback cases should stay at review/PDP level; stronger exact or support-backed cases may move naturally toward cart; multi-exact cases must not imply that one clear option already won.
