...
- **Cloudflare Pages Deploy Workflow Truth:** Commit `effbbce` (`chore(ci): make cloudflare pages deploy workflow manual`) is pushed and makes `.github/workflows/deploy-pages.yml` manual-only. The automatic `push` trigger for `main` was removed, while `workflow_dispatch` remains available for future controlled/manual use if `CLOUDFLARE_ACCOUNT_ID` / `CLOUDFLARE_API_TOKEN` are repaired. No job names, build commands, Wrangler command, env var names, secret names, or project name `vsm-store` changed. Cloudflare Pages native Git integration remains the active deploy path for now based on production manifest evidence serving `726222c`; the prior GitHub Actions run `25901261310` failed before build/deploy at `Verify Cloudflare credentials` because Cloudflare secrets were missing/unavailable/empty in the runner context. No deploy, workflow rerun, secret change, Cloudflare settings change, SQL, DB push/reset, remote Supabase operation, RPC call, order mutation, code change, migration change, or docs/canon change occurred during the workflow patch/push. Admin unpaid cancellation RPC switch remains closed/canonized; production admin observation remains blocked by auth; remote sandbox RPC smoke remains unresolved/blocked and must not be claimed as PASS.
