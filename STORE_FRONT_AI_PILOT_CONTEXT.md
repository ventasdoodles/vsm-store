...
3. **Verify:** Check if the assistant follows the Sommelier persona rules, asks one commercially useful narrowing question when needed, and only steers toward one option when the comparative support is real. If the shown products are too close, it should stay neutral instead of inventing a forced hierarchy before pushing toward a product card, PDP, or cart move.
