...
- **Retrieval Quality:** Historical search-leading recovery remains accepted and live-proven on its clean audited window, but current live semantic retrieval is on explicit operational hold until embeddings are repopulated after the accepted `768d` live migration. Present live state: `products` `44/0` embedded and `store_knowledge` `41/0` embedded because Gemini repopulation attempts hit `429 RESOURCE_EXHAUSTED`.
...
- **Current PRODUCT_SEARCH Operational Hold:** The vector/schema/RPC mismatch is no longer the blocker. The live environment already matches `768d` truth, but semantic retrieval remains operationally blocked because live embeddings are currently empty after strict down-migration and Gemini provider quota blocked repopulation. This is a quota-only operational hold, not a storefront-lane regression, not a routing/UI change, and not a closure claim.
...
- **Current Embedding Coverage:** live `products` currently stand at `44 active / 0 embedded` and live `store_knowledge` at `41 active / 0 embedded` after the accepted `768d` live migration nulled incompatible `3072d` vectors and Gemini repopulation retries hit `429 RESOURCE_EXHAUSTED`. This is operational-hold truth only, not a schema/RPC failure.
