...
- **Atomic Inventory Decrement & Work-Kit Evolution (2026-09-02):** Patched a severe system flaw where inventory was never decremented on checkout. Enforced `CHECK (stock >= 0)` on `products` and `product_variants`. Implemented `decrement_stock_for_order` PL/pgSQL function leveraging `SELECT ... FOR UPDATE` locks for race-condition prevention, correctly tied to the `mercadopago-webhook` idempotency contract. Added `implementation` lane to `vsm-gate.mjs` (which inherently runs `test:core` and `typecheck`), and optimized agent token usage via `bail: 1` and `test:changed` in Vitest. Repository validated with 929/929 passing tests.
