...
- **Current storefront recovery state:** Exact misses can recover through bounded token-based catalog matching when `requires_semantic_expansion === false`; embedding-driven semantic recovery remains a separate path. Drafting, UI labels, and telemetry now preserve that distinction truthfully via `capsule_match_strategy` and `capsule_retrieval_source`, while keeping the storefront handoff commercially useful.
