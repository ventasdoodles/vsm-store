...
- **Exact-Path Improvement: Context Lift + Fallback Naturalness** (implemented, approved for reconciliation): BRANCH C exact path now carries full product context (description + specs fields). Schema extended, exact query select enhanced, BRANCH C fallback logic tiered (ai_sales_note → specs → generic). Phrasing refined with Viene verb for natural Spanish ("...Viene con sabor menta..."). Real needed fix relative to committed HEAD (workspace drift resolved). No semantic lane reopening. Deployable within scope. Commit: 2b8be13. Audit: A71.
...
│
├── supabase/
│   ├── migrations/                  # 54 migraciones SQL (001 → 20260317_store_knowledge)
