...
- **AI Reliability / Evals / Operational Excellence — Phase 1 (DONE, ACCEPTED WITH EXPLICIT RESIDUAL RISK)**: The accepted Phase 1 scope was `Acceptance Harness Hardening + Golden Eval Baseline` only. The canonical authenticated harness (`npm run test:qa` -> `scripts/simulate_cesarin.ts`) is now accepted as standing regression infrastructure, and the final frozen baseline under `phase-1-strict` closed clean at `PASS: 9`, `DEGRADED: 0`, `FAIL: 0`, `BLOCKED: 0`. Canon truth is intentionally narrow: the final accepted recovered state is credited to the bounded runtime fixes culminating in Pass 2 + Pass 3 under the frozen harness, not to the earlier Pass 1 threshold/calibration inflation history. Residual risk remains explicit: this is not a storefront redesign, not a new commercial-behavior lane, not a new architecture/provider wave, not a claim of permanent immunity from runtime drift, and not a reliability proof beyond the defined Phase 1 suite.
- **Césarín Storefront — Storefront Conversational Checkout Execution Bridge (DONE, ACCEPTED)**: The storefront assistant now has one accepted bounded orchestration bridge from already-accepted checkout-readiness and authenticated order-tracking truth into already-existing storefront execution surfaces. The accepted implementation commit (`ca9b100de9d17c97184320f6b8ae3627fbac585f`) attaches route handoff only through the existing `message.action` surface in `src/services/concierge.service.ts`; the product-only `next_step_view` contract remains unchanged. Accepted eligible handoffs are exact and narrow: `Abrir checkout` -> `/checkout`, `Retomar orden abierta` -> `/orders/{id}`, and `Continuar pago pendiente` -> `/payment/pending?order_id={id}`. Non-eligible checkout/order states remain advisory-only. This is orchestration-only: not checkout redesign, not a payment/provider architecture wave, not autonomous payment execution, not a storefront visual redesign, and not a new action schema beyond the existing CTA behavior.
- **Césarín Storefront — Storefront AI Pilot Gate Graduation & Controlled All-User Exposure (DONE, ACCEPTED)**: Storefront exposure is now truthfully centered on the existing global storefront flag instead of the retired dual-gate requirement. The accepted exposure contract is exact and bounded: `global off + pilot off => hidden`, `global off + pilot on => visible via bounded pilot preview/QA override`, `global on + pilot off => visible to ordinary storefront users`, and `global on + pilot on => visible to ordinary storefront users, with pilot retained only as debug/access-path context`. Canon truth is explicit that the old `global AND pilot` rule is no longer the storefront reality and that pilot authorization now remains only as bounded preview/QA override when global exposure is off. This lane is exposure-gating only: not runtime/prompt/capsule/routing redesign, not checkout/payment/provider work, not PRODUCT_SEARCH tuning, and not storefront redesign. The observed `degraded-ux-timeout-01` failure during the standing gate was acceptance-audited as unrelated runtime latency noise outside this front's causality.
- **Cesarin OS — Governed Operator Queue Convergence (DONE, ACCEPTED)**: Cesarin OS operator follow-up now converges on `cesarin_improvement_items` as the canonical governed work queue. Accepted implementation source: commit `2e89915a53b6d8a83bd97559c4c73ba3ac56c795`. The accepted queue contract is exact and admin-only: reviewed real interactions enter via `analytics_id`; approved intervention recommendations enter via `intervention_recommendation_id` plus `intervention_signal_id`; `TabLearning` and `TabInterventions` are intake/handoff surfaces; `TabImprovements` is the persisted closure queue for ownership, execution notes, artifact evidence, status, and closure. This fixes operator workflow fragmentation without claiming storefront/runtime/prompt/capsule/search changes, checkout/payment/provider changes, giant admin redesign, or autonomous intervention execution. Residual design note: minor schema tension remains around FK `on delete set null` versus required lineage fields, but this is not a blocker.
- **Conversational Conversion Intelligence — Phase 1: Measurement-to-Decision Readout (DONE, ACCEPTED WITH EXPLICIT RESIDUAL RISK)**: Cesarin OS now records one bounded, read-only, production-grade conversion readout inside the existing analytics surface. Accepted implementation source: commit `b4cb44744102b57adbc3572908d4baa88dbc9e05`. Accepted data surfaces are exact: `conversation_conversion_events`, `orders`, and `products`. Accepted output shape is session-level funnel reconstruction, aggregate counts by event type, source counts, CTA kind counts, cart mutation result counts, drop-off counts, and product-level summaries. This readout is structurally active and enables observation and decision-making from existing conversion events and order attribution, but early live data may still be sparse and the front does not itself optimize conversion. Non-claims remain explicit: no optimization logic, no prompt/routing/capsule changes, no checkout/payment/provider changes, no exposure rule changes, no operator queue logic changes, and no dashboard/platform overbuild. Residual risk is accepted and non-blocking: product-level order attribution is metadata-backed rather than full order-item attribution, and some edge-case truth labels are code-proven rather than directly asserted in focused tests.
- **Conversational Conversion Intelligence — Probe Readout Filter (DONE, ACCEPTED WITH MINOR RESIDUAL RISK)**: The accepted read-only conversion readout now has one bounded hygiene filter for activation/probe traffic. Accepted implementation source: commit `ee841def00c7fc23c5696a8207e88c9d94e3b3de` (`fix: filter probe traffic from conversion readout`). Probe-marked traffic is excluded from the default commercial conversion readout, and a compact excluded-probe summary is exposed in the existing admin analytics surface. This is bounded to the admin conversion readout path only: no storefront behavior, checkout/payment/provider logic, prompt/routing/capsule logic, schema, docs, or broader analytics platform work changed in the implementation pass. Minor residual risk remains explicit: `activation_probe: 'true'` is implemented but not directly test-covered; `excludedProbeSessions` may not count every indirectly excluded session when exclusion happens only via order-id linkage; probe product ids may still be queried from product lookup even though they do not enter commercial counts or product summaries; and unmarked synthetic traffic remains indistinguishable from ordinary traffic.
...
- **Accepted operating discipline:** The resolver is strict read-only and reuses existing storefront truth only: cart truth, checkout draft truth, payment settings truth, address truth, coupon validation truth, and authenticated open-order recovery truth. The lane stays bounded to readiness states such as `READY_TO_CHECKOUT`, `MISSING_REQUIRED_INFO`, `CART_BLOCKER`, `PAYMENT_METHOD_INFO`, `SHIPPING_INFO_AVAILABLE`, `SHIPPING_INFO_PARTIAL`, and `AUTH_REQUIRED`. Responses remain non-catalog and advisory by default, with later accepted route handoff limited to the separate bounded checkout-execution bridge only for eligible `/checkout` or `/orders/{id}` continuation. Shipping-cost guidance stays bounded to what current storefront truth can actually support.
...
- **Accepted storefront lane:** `Storefront Conversational Checkout Execution Bridge` is now canonized as `ACCEPT`.
- **Accepted implementation commit:** `ca9b100de9d17c97184320f6b8ae3627fbac585f` (`feat: bridge concierge checkout execution handoffs`).
- **Scope:** `src/services/concierge.service.ts`, `src/services/__tests__/concierge.service.execution-bridge.test.ts`, `src/components/ui/ai/__tests__/AIConcierge.test.tsx`, and the existing assistant CTA surface already present in `src/components/ui/ai/AIConcierge.tsx`. This lane is orchestration exposure only over already-existing storefront checkout/order/payment continuation surfaces.
- **Accepted truth:** Césarín storefront now supports one bounded route-handoff bridge from accepted checkout-readiness and authenticated order-tracking truth into existing storefront execution surfaces through the existing `message.action` surface only. The accepted eligible route handoffs are exact: `Abrir checkout` -> `/checkout`, `Retomar orden abierta` -> `/orders/{id}`, and `Continuar pago pendiente` -> `/payment/pending?order_id={id}`.
- **Accepted operating discipline:** CTA eligibility remains strict and allowlist-based. Checkout handoff is allowed only for `READY_TO_CHECKOUT` with `can_proceed_to_checkout === true`, `can_submit_checkout === true`, `blocker_reason === 'none'`, and no open order. Open-order continuation is allowed only for `CART_BLOCKER` with `blocker_reason === 'open_recoverable_order'` plus `open_order_id`. Payment continuation is allowed only for `FOUND` order-tracking truth with `payment_method === 'mercadopago'`, `payment_status === 'pending'`, and `order_status !== 'cancelled'`. All other checkout/order states remain advisory-only. The product-only `next_step_view` contract was not expanded in this lane.
- **Residual truth safeguards / explicit non-claims:** This lane does not claim checkout redesign, payment/provider architecture work, assistant-created orders, assistant-triggered payment creation, autonomous payment execution, `next_step_view` expansion, storefront visual redesign, or any ranking/catalog behavior change. Residual risk remains explicit: this is route handoff through the existing CTA behavior, not in-chat autonomous payment execution.
- **What did not change:** Prior storefront/core lanes remain authoritative and non-reopened. This lane does not mutate checkout/provider logic, does not redesign payment flow, does not replace existing storefront payment re-entry mechanisms, and does not convert Césarín into an autonomous checkout/payment surface.
- **Outcome:** Césarín can now truthfully expose one bounded `message.action` link into existing `/checkout`, `/orders/{id}`, and `/payment/pending?order_id={id}` surfaces when eligibility is grounded by existing capsule truth, while non-eligible states remain advisory-only.

### Storefront AI Pilot Gate Graduation & Controlled All-User Exposure - 15 de abril de 2026
- **Accepted storefront lane:** `Storefront AI Pilot Gate Graduation & Controlled All-User Exposure` is now canonized as `ACCEPT`.
- **Scope:** `src/App.tsx`, `src/lib/pilot-activation.ts`, `src/pages/admin/AdminCesarinOS.tsx`, and tightly relevant gate/debug tests only. This lane is exposure-gating truth only.
- **Accepted truth:** The storefront no longer uses the old dual-gate requirement as its exposure truth. The accepted contract is exact: `global off + pilot off => hidden`, `global off + pilot on => visible via bounded pilot preview/QA override`, `global on + pilot off => visible to ordinary storefront users`, and `global on + pilot on => visible to ordinary storefront users, with pilot retained only as debug/access-path context`.
- **Accepted operating discipline:** `is_ai_assistant_enabled` is now the authoritative storefront exposure flag for ordinary users. Pilot authorization remains only as a bounded preview/QA override when global exposure is off, plus debug/access-path context when global exposure is already on. Admin/operator wording and visible debug state must describe that truth directly rather than restating the retired `global AND pilot` rule.
- **Residual truth safeguards / explicit non-claims:** This lane does not claim runtime/prompt/capsule/routing redesign, checkout/payment/provider work, PRODUCT_SEARCH tuning, compatibility/kitting expansion, or storefront redesign. It records exposure-gating truth only.
- **Causality note:** The observed `degraded-ux-timeout-01` gate failure was acceptance-audited as unrelated runtime latency noise outside this front's causality.
- **What did not change:** Prior storefront/core lanes remain authoritative and non-reopened. Assistant behavior, capabilities, prompts, checkout routes, and storefront design are not broadened by this entry.
- **Outcome:** Canon truth now matches the accepted storefront reality: global-on exposes Césarín to ordinary storefront users, while pilot authorization remains only as bounded preview/QA override when global exposure is off.

### Cesarin OS Governed Operator Queue Convergence - 17 de abril de 2026
- **Accepted admin lane:** `Cesarin OS Governed Operator Queue Convergence` is now canonized as `ACCEPT`.
- **Accepted implementation source:** commit `2e89915a53b6d8a83bd97559c4c73ba3ac56c795`.
- **Scope:** Admin-only operator workflow convergence across the existing governed surfaces: `PilotTelemetry`, `ReviewDrawer`, `TabLearning`, `TabInterventions`, `TabImprovements`, `admin-improvement.service.ts`, `admin-improvement-workflow.service.ts`, the bounded migration for persisted lineage, and tightly relevant admin workflow tests. This front does not reopen storefront/runtime/search/checkout lanes.
- **Accepted queue contract:** `cesarin_improvement_items` is the canonical governed operator work queue. Reviewed real interactions enter the queue via `analytics_id`. Approved intervention recommendations enter via `intervention_recommendation_id` plus `intervention_signal_id`. `TabLearning` and `TabInterventions` are intake/handoff surfaces. `TabImprovements` is the persisted closure queue where ownership, execution notes, artifact evidence, status, and closure live.
- **Accepted operating discipline:** `createImprovementItemFromRecommendation(...)` creates or reuses canonical queue items. `TabInterventions` approval executes the accepted handoff chain: `recordOperatorDecision(...)`, `createImprovementItemFromRecommendation(...)`, then `acknowledgeSignal(...)`. `TabImprovements` owns persisted operator closure rather than leaving approved recommendations as a separate dead-end workflow.
- **Residual truth safeguards / explicit non-claims:** This lane does not claim storefront/runtime/prompt/capsule/search changes, checkout/payment/provider changes, a giant admin redesign, or autonomous intervention execution. `TabLearning` and `TabInterventions` remain intake/handoff surfaces rather than parallel closure systems.
- **Residual design note:** Minor schema tension remains around FK `on delete set null` versus required lineage fields. This is a residual design note, not an acceptance blocker.
- **Gate truth recorded by audit:** The standing regression gate was green for the accepted implementation: `PASS: 9`, `DEGRADED: 0`, `FAIL: 0`, `BLOCKED: 0`.
- **Outcome:** Operator workflow fragmentation is fixed by converging follow-up into one persisted governed queue while keeping the front bounded to admin workflow convergence only.

### Conversational Conversion Intelligence - Probe Readout Filter - 22 de abril de 2026
- **Accepted hygiene pass:** `Conversational Conversion Intelligence - Probe Readout Filter` is now canonized as `ACCEPT WITH MINOR RESIDUAL RISK`.
...
- **Scope:** Admin conversion readout path only. The pass keeps the accepted Phase 1 readout read-only and prevents probe/activation windows from being mixed invisibly into ordinary commercial interpretation.
- **Accepted behavior:** Probe-marked traffic is excluded from the default commercial conversion readout, and the existing admin analytics surface exposes a compact excluded-probe summary. The pass is a readout hygiene filter, not optimization logic and not a broader analytics platform.
- **What did not change:** No storefront behavior, checkout/payment/provider logic, prompt/routing/capsule logic, schema, docs, or broader dashboard platform work changed in the implementation pass. Closed fronts remain closed.
- **Residual truth safeguards / explicit non-claims:** This does not claim all synthetic traffic is solved, does not claim causal conversion uplift, and does not claim commercial optimization impact. `activation_probe: 'true'` is implemented but not directly test-covered; `excludedProbeSessions` may not count every indirectly excluded session when exclusion happens only via order-id linkage; probe product ids may still be queried from product lookup even though they do not enter commercial counts or product summaries; and unmarked synthetic traffic remains indistinguishable from ordinary traffic.
- **Outcome:** Ordinary commercial readout interpretation is protected from explicitly marked probe traffic while preserving the bounded, read-only Measurement-to-Decision scope.
...
- **Accepted scope truth:** This accepted lane covers `Acceptance Harness Hardening + Golden Eval Baseline` only. It does not reopen storefront UX, commercial behavior, Prompt Ops, Gemini Runtime Wave 1, or a new provider/architecture front.
- **Accepted harness truth:** The authenticated Phase 1 harness is now the standing regression gate for this scope. The canonical execution path is `npm run test:qa` through `scripts/simulate_cesarin.ts`, using the frozen scenario manifest and threshold pack already accepted earlier in the lane.
- **Accepted recovery truth:** After bounded runtime triage under the frozen harness, the final canonical baseline closed at `PASS: 9`, `DEGRADED: 0`, `FAIL: 0`, `BLOCKED: 0`.
- **Attribution truth:** Pass 1 contained threshold/calibration inflation history and is not credited as the main recovery signal. The accepted recovered final state is specifically credited to bounded runtime fixes culminating in Pass 2 + Pass 3 under the unchanged harness/scenario/threshold gate.
- **Residual truth safeguards / explicit non-claims:** This acceptance does not claim permanent immunity from future regressions, full-stack or perpetual reliability proof, storefront redesign, new commercial ranking behavior, or a new architecture/provider modernization wave. Ordinary operational/runtime drift remains possible and the accepted gate must continue to be used for future regression checks.
- **Outcome:** `AI Reliability / Evals / Operational Excellence` Phase 1 is now accepted in canon with explicit residual risk. The harness is accepted as standing infrastructure, the recovered final baseline is accepted for the defined Phase 1 suite, and closure applies to this bounded Phase 1 outcome only.
...
*Actualizado: 17 de abril de 2026 (Cesarin OS Governed Operator Queue Convergence - ACCEPT).*
*Actualizado: 21 de abril de 2026 (Conversational Conversion Intelligence - Phase 1 Measurement-to-Decision Readout - ACCEPT WITH EXPLICIT RESIDUAL RISK).*
*Actualizado: 22 de abril de 2026 (Conversational Conversion Intelligence - Probe Readout Filter - ACCEPT WITH MINOR RESIDUAL RISK).*
