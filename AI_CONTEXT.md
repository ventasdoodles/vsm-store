...
- **Cesarin OS Repo Graph Subview** (implemented, accepted, closed): `Conceptos` now includes a bounded local mode switch between the existing compatibility tooling and a read-only repo graph inspector. Scope stayed inside `TabConcepts`; no new top-level Cesarin OS tab was created. `admin-repo-graph.service.ts` statically consumes local `graqle.json` and exposes read-only helpers for node search, direct graph relations, nearby same-container nodes, and chunk previews. `TabRepoGraph.tsx` renders search, type filter, node metadata, direct relations, nearby containment neighbors, and chunk previews, with explicit truth labels: read-only, local `graqle.json` consumption, no live backend, and no runtime dependency proof. Acceptance audit confirmed the surface is operator-usable without overstating graph certainty. Follow-up copy-only hygiene pass tightened `TabConcepts` wording so compatibility mode no longer leaks repo-graph vocabulary. Commits: 824a2ed (repo graph subview), 99672e9 (copy tightening). Audit: A91.
