...
function fixtureResponseFor(model, prompt, iteration) {
  ...
  return {
    ...
    rawText: JSON.stringify({
      ...
      fallback_reason: toolCalls.length > 0 ? null : 'NO_CAPSULE_MATCH',
    }),
    source: 'fixture',
  };
}
...
function evaluateParsedOutput(prompt, parsed) {
  const expected = prompt.expected ?? {};
  ...
  for (const modelId of args.models) {
    for (let iteration = 1; iteration <= args.iterations; iteration += 1) {
      for (const prompt of prompts) {
        ...
        const evaluation = evaluateParsedOutput(prompt, parsed);
        ...
        results.push({
          ...
          fallbackTriggered: evaluation.intent === 'support' || evaluation.flags.includes('missing_text'),
